...
class Database(object):
    ...
    def export_csv(self, dir):
        try:
            import csv
            heading = ['students ID', 'first', 'last', 'year', 'students note', 'visit_date', 'visit_start', 'show', 'topic', 'visits note', 'comments', 'observations', 'recommendations']

            all_data = self.cur.execute(\
            'select students.ID, first, last, year, students.note, visit_date, visit_start, show, topic, visits.note, comments, observations, recommendations\
            from students left outer join visits on students.ID = visits.ID;'\
            ).fetchall()
    ...
    def resetDB(self):
        try:
            ...
            self.cur.execute('delete from visits')
        except sqlite3.IntegrityError, value:
            logging.warning(value)
            #raise exception for the GUI
            raise Exception(value)
